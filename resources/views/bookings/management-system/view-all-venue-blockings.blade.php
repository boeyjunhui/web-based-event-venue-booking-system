    ...
    <div class="p-4 bg-white rounded-lg mt-2">
        <div class="grid grid-cols-2 mb-6 items-center">
            <div class="grid justify-items-start">
                <h1 class="content-center text-2xl font-bold text-gray-800 mt-2 mb-2">Venue Blockings</h1>
            </div>

            <div class="grid justify-items-end">
                <a href="/evbs/bookings/add-venue-blocking"><button type="submit" class="p-1 w-16 text-2xl text-white rounded-md bg-teal-500 hover:bg-teal-700 transition"><i class="bx bx-plus"></i></button></a>
            </div>
        </div>
            ...
            <div class="overflow-x-scroll mb-6">
                <table id="data-table" class="text-sm whitespace-nowrap">
                    ...
                    <tbody class="text-gray-800 bg-neutral-50">
                        ...
                        @forelse ($venueBlockings as $venueBlocking)
                            <tr class="hover:bg-neutral-200 transition">
                                ...
                                <td>{{ $venueBlocking->start_date }}</td>
                                <td>{{ $venueBlocking->end_date }}</td>
                                <td>{{ $venueBlocking->start_time }}</td>
                                <td>{{ $venueBlocking->end_time }}</td>
                                <td>
                                    @if ($venueBlocking->status == 0)
                                ...
                                <td>{{ $venueBlocking->created_at }}</td>
                                <td>{{ $venueBlocking->updated_at }}</td>
                            </tr>
                            ...
                            <script>
                                document.getElementById('delete-confirmation-{{ $no }}')?.addEventListener('click', function () {
                                    ...
                                    Swal.fire({
                                        titleText: 'Delete',
                                        text: "Are you sure you want to delete this venue blocking?",
                                        icon: 'warning',
