...
@section('content')
    ...
    <div class="p-4 bg-white rounded-lg mt-6">
        ...
        <form action="/evbs/event-venue-owners/{{ $eventVenueOwner->id }}/update-password" method="POST" enctype="multipart/form-data">
            @csrf
            @method('PATCH')
            ...
            <div class="grid grid-cols-2 gap-8 mt-20 mb-6">
                ...
                <div class="grid justify-items-start">
                    <a href="/evbs/event-venue-owners/{{ $eventVenueOwner->id }}" class="p-2 w-40 text-center text-sm text-white rounded-lg bg-red-500 hover:bg-red-700 transition">Cancel</a>
                </div>
            </div>
        </form>
    </div>
